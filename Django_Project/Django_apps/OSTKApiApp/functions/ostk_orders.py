...
# *********************************ALL********************************************
def ostk_open_orders(request):
    conn = mysql_connection()
    cursor = conn.cursor()
    orders_q = """SELECT order_code, reference_no, carrier_code, service_level_code, item, qty, tracking_no, status, wh_code
    FROM orders_record
    WHERE status = 'O'"""
    cursor.execute(orders_q)
    data = cursor.fetchall()
...
def ostk_processed_orders(request):
    conn = mysql_connection()
    cursor = conn.cursor()
    orders_q = """SELECT order_code, reference_no, carrier_code, service_level_code, item, qty, tracking_no, status, wh_code
    FROM orders_record
...
def ostk_open(request):
    conn = mysql_connection()
    conn_wms = wms_mysql_connection()
    conn_cloud = cloud_connection()
    orders_q = """SELECT order_code, reference_no, carrier_code, service_level_code, item, qty, tracking_no, retail_order_no, add_time AS received, wh_code
    FROM orders_record
    WHERE status != 'S' AND status != 'H' AND status != 'X'"""
    wms_q = """SELECT order_code, order_status FROM wms.orders WHERE warehouse_id = 11 AND customer_code = 'OSTK'"""
    cloud_q = """SELECT tracking_number AS tracking_no, create_date AS scan_time FROM itweb.web_scan_detail where username = 'YoskardyS'"""
    orders_df = pd.read_sql_query(orders_q, conn)
    wms_df = pd.read_sql_query(wms_q, conn_wms)
...
def ostk_export(request):
    conn = mysql_connection()
    conn_wms = wms_mysql_connection()
    conn_cloud = cloud_connection()
    orders_q = """SELECT order_code, reference_no, carrier_code, service_level_code, item, qty, tracking_no, retail_order_no, status AS api_status, add_time AS received, wh_code
    FROM orders_record"""
    wms_q = """SELECT order_code, order_status FROM wms.orders WHERE warehouse_id = 11 AND customer_code = 'OSTK'"""
    cloud_q = """SELECT tracking_number AS tracking_no, create_date AS scan_time FROM itweb.web_scan_detail where username = 'YoskardyS'"""
    orders_df = pd.read_sql_query(orders_q, conn)
